...
pub async fn select_users() -> (StatusCode, Json<ServerResponse<Vec<User>>>) {
    ...
    let users = user.select(User::as_select()).load(&mut *db);
...
pub async fn create_user(
    ...
    if let Err(_e) = users {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            ServerResponse::error("Failed to Create User"),
        );
    }
...
pub async fn authenticate(
    ...
    let auth_user_data: Result<User, diesel::result::Error> =
        user.filter(email.eq(&body.email)).first::<User>(db);

    if let Err(_e) = auth_user_data {
        return (
            StatusCode::BAD_REQUEST,
            ServerResponse::error("Email is wrong or User does not exist!"),
        );
    }
    ...
    if !verify {
        return (
            StatusCode::BAD_REQUEST,
            ServerResponse::error("Password is Wrong!"),
        );
    }
...
pub async fn token_verification(
    Json(body): Json<TokenVerification>,
) -> (StatusCode, Json<ServerResponse<String>>) {
    ...
    if let Err(_e) = verified_tokens {
        return (
            StatusCode::BAD_REQUEST,
            ServerResponse::error("Token not valid!"),
        );
    }
